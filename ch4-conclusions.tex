...
The January 2026 US East Coast winter storm provides a natural experiment for calibrating the $\beta$ channel. Systematic collection of post-event data---downtime, revenue impact, insurance claims---would enable model validation.

\subsubsection*{Water-energy-climate nexus}

Liquid cooling reduces energy but increases water consumption. In water-stressed regions, this may exchange one vulnerability for another. Integrating water projections with cooling trajectories is relevant as AI facilities adopt liquid cooling at scale.

\subsubsection*{Real options for adaptation timing}

A real-option extension would value flexibility to delay while preserving ability to act. The 15--20\% vs 40--60\% CAPEX differential between greenfield and retrofit suggests significant option value in early commitment.

\subsubsection*{Expected Shortfall and tail risk}

Extending to Expected Shortfall (CVaR) would provide a metric better suited to insurance pricing and prudential regulation, where tail losses dominate. Mathematical foundations in Appendix~A.

\subsubsection*{Cross-sector generalization}

The ClimVaR + Shapley + Leontief combination is transferable. Manufacturing, energy, and transportation face analogous challenges. Extension to multi-sector portfolios---data centers $\times$ grids $\times$ telecom---would capture systemic risks.

\subsubsection*{System Dynamics Modeling}

A natural extension: time-varying pathways where carbon prices respond to policy, operator strategies adapt to losses, and MRIOT coefficients reflect supply chain restructuring \citep{beaufils2025}.
