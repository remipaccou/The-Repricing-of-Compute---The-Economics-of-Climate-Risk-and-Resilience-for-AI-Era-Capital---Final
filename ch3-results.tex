...
\noindent The largest announced AI facility carries 46.5\% ClimVaR intensity. Business interruption (43\%) and cooling (37\%) dominate; carbon and heat are marginal. The Shapley decomposition shows 56\% of BI risk comes from tropical wind propagating through semiconductor supply chains. A facility in the US South derives most BI exposure from cyclones thousands of kilometers away. Direct on-site hazards: $<$1\% of BI.

\subsubsection*{Site~B (France) — The Flood Trap}

\noindent A temperate European site carries 167\% ClimVaR intensity---risk exceeds enterprise value. River flood (\$2.7B, 75\%) dominates. Without granular attribution, generic assessments classify this as low-risk. The 167\% is a screening signal: flood exposure merits engineering assessment with site-specific data on elevation, defenses, and drainage.

\subsubsection*{Site~C (Southern China) — The Triple Threat}

\noindent Site~C's 37.6\% intensity masks complexity: BI (53\%), carbon (22\%), cooling (18\%) form a three-pronged profile. No single lever reduces more than a quarter of exposure. Effective risk reduction requires a \textit{multi-channel} package: microgrids for BI, PPAs for carbon, high-efficiency cooling for thermal loads.

\subsubsection*{From portfolio to site-level decisions}

\noindent The same ClimVaR, Shapley, and Leontief machinery applies at site level. For Site~B, local flood defense data can recalibrate the 167\% signal. For Site~A, supply chain audits refine Leontief coefficients. The framework answers: \textit{which channel dominates, which hazards drive it, which interventions work fastest?}
